...
//新增光路拓扑图(Routes,传入光路拓扑图：局站id)
	public List<Map<String,Object>> addTopology(List<Serializable> ids){   
		...
		while(iterator.hasNext()){
			int crossStation =stationNumber-2;//跨站数
			...
			int index=1; //当前局站所在ids中编号
			while(crossStation>=0){//一直向下寻找，直到找完所有的节点
				if(fiber_core!=null){//存在满足条件的纤芯
					routeMap.add(fiber_core.getId());//设置第二个路由线路：光纤
					//如果该光纤A端和光路跳纤同时连接在同一个配线架上，则应该将光纤Z端配线架加入下一个站点
					//寻找光纤另一端的配线架信息
					long siteFrameId=fiber_core.getFrame_z_id();
					int siteFramePort=fiber_core.getPort_order_z();
					if(fiber_core.getFrame_z_id() == jumper_route.getFrame_id()){//说明光纤的A端是另一端
						siteFrameId=fiber_core.getFrame_a_id();
						siteFramePort=fiber_core.getPort_order_a();
					}
					pointMap.add(siteFrameId);
					if(crossStation==0){
						break;
					}
					Jumper_frames siteJumperFrame=jumperFrameDao.findByFrameIdandPortId(siteFrameId, siteFramePort);
					if(siteJumperFrame!=null){//当前Z站存在配线架跳纤
						routeMap.add(siteJumperFrame.getId());//放入配线架跳纤节点
						//配线架跳纤的另一端配线架及其端口，初步认为Z端为另一端
						long belowFrameId=siteJumperFrame.getFrame_z_id();
						int belowFramePort=siteJumperFrame.getPort_order_z();
						if(siteJumperFrame.getFrame_z_id()==siteFrameId){//a端为另一端
							 belowFrameId=siteJumperFrame.getFrame_a_id();
							 belowFramePort=siteJumperFrame.getPort_order_a();
						}
						pointMap.add(belowFrameId);//放入配线架节点
						//寻找下一段光纤
						fiber_core = fiberCoreDao.findByFrameIdandPortOrder(belowFrameId,belowFramePort);
						index++;//已经开始寻找下一个站
						crossStation--;
						if(fiber_core!=null){
							siteFrameId=fiber_core.getFrame_z_id();
							siteFramePort=fiber_core.getPort_order_z();
							long startStationId=fiber_core.getStation_a_id();//当前光纤的A、Z站点，判断是否与用户输入的拓扑相同
							long endStationId=fiber_core.getStation_z_id();
							if(fiber_core.getFrame_z_id()==jumper_route.getFrame_id()){//说明光纤的A端是另一端
								siteFrameId=fiber_core.getFrame_a_id();
								siteFramePort=fiber_core.getPort_order_a();
								startStationId=fiber_core.getStation_z_id();
								endStationId=fiber_core.getStation_a_id();
							}
							//必须满足纤芯的起始端与站点拓扑信息相同，如果不同。跳出当次循环
							if(startStationId!=(Long)ids.get(index-1)||(endStationId!=(Long)ids.get(index))){
								break;
							}
						}
					}
...
//新增光功率数据记录
	public synchronized Serializable addOpticalPower(Optical_powers optical_power ){
		return opticalPowerDao.addEntity(optical_power);
	}
//新增告警信息
	public synchronized Serializable addAlarm(Alarm alert){
